...
def create_contact_map(structure, threshold=8.0):
    ...
    # tworzymy listê atomów CA dla ka¿dej reszty
    ca_atoms = [residue['CA'] for residue in chain if 'CA' in residue]

    #porównujemy ka¿dy atom z ka¿dym
    #unikamy przypadków gdzie i >= j, aby unikn¹æ powtarzania porównanñ
    for i, ca1 in enumerate(ca_atoms):
        for j, ca2 in enumerate(ca_atoms):
            if i >= j:
                continue

            # Obliczmy odleg³oœci miêdzy dwoma atomami CA
